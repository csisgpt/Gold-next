        ...
        {items.map((item, index) => (
          <SwiperSlide key={`${item.name}-${index}`}>
            <article className="product-card">
              <div className="product-card-image"><Image src={item.image} alt={item.name} fill sizes="320px" />{item.badge ? <span className="product-badge">{item.badge}</span> : null}<span className="catalog-sample">نمونه کاتالوگی</span></div>
