...
export default function RegisterClient({ intent = "register", metal = "" }: { intent?: string; metal?: string }) {
    ...
    <main className="register-shell">
      ...
      <section className="register-section"><div className="container register-wrapper">
        <form className="register-card" onSubmit={handleSubmit}>
          <input type="hidden" name="intent" value={intent} /><input type="hidden" name="metal" value={metal} />
          <div className="register-tab">{copy.tab}</div><h1>{copy.title}</h1><p>{copy.text}</p>
          ...
          <button className="cta-button primary-btn full-width-button">{copy.button}</button>{message ? <div className="submit-message">{message}</div> : null}
        </form>
      </div></section>
